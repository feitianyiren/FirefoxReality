...
struct DeviceDelegateOpenXR::State {
  ...
  OpenXRSwapChainPtr boundEyeSwapChain;
  ...
  void UpdateSpaces() {
    if (viewSpace != XR_NULL_HANDLE && localSpace != XR_NULL_HANDLE && stageSpace != XR_NULL_HANDLE) {
      return;
    }
    CHECK(session != XR_NULL_HANDLE);
...
DeviceDelegateOpenXR::BindEye(const device::Eye aWhich) {
  ...
  if (m.boundEyeSwapChain) {
    m.boundEyeSwapChain->ReleaseImage();
  }
...
DeviceDelegateOpenXR::LeaveVR() {
  CHECK_MSG(!m.boundEyeSwapChain, "Eye swapChain not released before LeaveVR");
  ProcessEvents();
  m.vrReady = false;
}
