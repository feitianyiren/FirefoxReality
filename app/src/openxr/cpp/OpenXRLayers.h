...
public:
  ...
  virtual bool IsDrawRequested() const override {
    return layer->IsDrawRequested() &&
       ((IsSwapChainReady() && IsComposited()) || layer->GetClearColor().Alpha() > 0.0f);
  }
...
protected:
  ...
  XrSwapchainCreateInfo GetSwapChainCreateInfo(uint32_t width, uint32_t height) {
...
private:
  void InitSwapChain(JNIEnv* aEnv, XrSession session, OpenXRSwapChainPtr &swapChainOut) {
    ...
    XrSwapchainCreateInfo info = this->GetSwapChainCreateInfo(this->layer->GetWidth(), this->layer->GetHeight());
    if (this->layer->GetSurfaceType() == VRLayerQuad::SurfaceType::AndroidSurface) {
      ...
      swapChainOut->InitFBO(render, session, info, device::RenderMode::StandAlone);
    }
  }
};
